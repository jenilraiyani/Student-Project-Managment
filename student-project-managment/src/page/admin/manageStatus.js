...
const ManageStatus = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (response.ok) {
        setStatuses(statuses.filter(s => s.taskStatusID !== id));
      } else {
        console.error('Failed to delete status');
        alert('Failed to delete the status. It might be in use.');
      }
    } catch (error) {
  ...
  const submitEdit = async () => {
    try {
      ...
      if (response.ok) {
        setStatuses(
          statuses.map(s =>
            s.taskStatusID === editingStatus.taskStatusID ? editingStatus : s
          )
        );
  ...
  const filteredStatuses = statuses.filter(item =>
    (item.taskStatusName && item.taskStatusName.toLowerCase().includes(searchTerm.toLowerCase())) ||
    (item.taskStatusCssClass && item.taskStatusCssClass.toLowerCase().includes(searchTerm.toLowerCase()))
  );
    ...
    <>
      <div className="container-fluid p-0">
        ...
        <div className="d-flex justify-content-between align-items-center mb-4">
          <div>
            <h4 className="fw-bold mb-1 text-dark" style={{ letterSpacing: '-0.5px' }}>Manage Status</h4>
            <nav aria-label="breadcrumb">
              <ol className="breadcrumb mb-0" style={{ fontSize: '0.85rem' }}>
                ...
                <li className="breadcrumb-item active fw-semibold" aria-current="page">Status</li>
              </ol>
            </nav>
          </div>
        </div>
        ...
        <div className="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4 gap-3">
          ...
          <button className="btn spms-btn-primary d-flex align-items-center gap-2 shadow-sm" onClick={() => navigate('/add-status')}>
              ...
              <table className="table table-hover align-middle spms-table mb-0">
                <thead>
                  <tr>
                    ...
                    <th scope="col" className="text-muted fw-bold text-uppercase" style={{ width: '35%' }}>Status Name</th>
                ...
                <tbody>
                  {filteredStatuses.length > 0 ? (
                    filteredStatuses.map((item, index) => {
                      const colors = getColorForClass(item.taskStatusCssClass);
                      return (
                        <tr key={item.taskStatusID} className="spms-table-row">
                          <td className="fw-semibold text-secondary">{index + 1}</td>
                          <td>
                            <span className="fw-bold text-dark">{item.taskStatusName}</span>
                          </td>
                          <td>
                            <div className="d-flex align-items-center gap-3">
                              <span className="text-secondary small fw-medium font-monospace">{item.taskStatusCssClass}</span>
                              <span
                          ...
                          <td className="text-center">
                            ...
                            <button className="btn btn-sm btn-light text-danger action-btn" title="Delete" onClick={() => handleDelete(item.taskStatusID)}>
                              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16">
                    ...
                    <tr>
                      <td colSpan="4" className="text-center py-5">
                        <div className="text-muted">
                          <h6 className="mt-3 fw-semibold text-dark">No Statuses Found</h6>
          ...
          <div className="spms-modal-overlay">
            <div className="spms-modal-content card spms-premium-card border-0">
              ...
              <div className="card-header bg-white border-bottom-0 pt-4 pb-2 px-4 d-flex justify-content-between align-items-center">
                <h5 className="fw-bold text-dark mb-0">Edit Status</h5>
              ...
              <div className="card-body px-4 pb-4">
                <div className="mb-4">
                  ...
                  <input
                    ...
                    value={editingStatus.taskStatusName}
                    onChange={(e) => setEditingStatus({ ...editingStatus, taskStatusName: e.target.value })}
                    required
                  />
                </div>
                ...
                <div className="mb-4">
                  <label className="form-label fw-bold text-secondary small">
                    Status CSS Class <span className="text-danger">*</span>
                  </label>
                  <select
                    className="form-select spms-input shadow-none"
                    value={editingStatus.taskStatusCssClass}
                    onChange={(e) => setEditingStatus({ ...editingStatus, taskStatusCssClass: e.target.value })}
                    required
                  >
                    <option value="warning">Warning (Orange - Pending)</option>
                    <option value="info">Info (Cyan - In Progress)</option>
                    <option value="success">Success (Green - Completed)</option>
                    <option value="danger">Danger (Red - Rejected/Overdue)</option>
                    <option value="primary">Primary (Blue - Open)</option>
                    <option value="dark">Dark (Black - On Hold)</option>
                ...
                <div className="p-3 bg-light rounded-3 mb-4 border d-flex align-items-center justify-content-between" style={{ borderColor: '#F1F5F9' }}>
                  ...
                  <span
                    className="priority-badge-preview"
                    style={{
                      backgroundColor: getColorForClass(editingStatus.taskStatusCssClass).bg,
                      color: getColorForClass(editingStatus.taskStatusCssClass).text,
                      borderColor: getColorForClass(editingStatus.taskStatusCssClass).border
                    }}
                  >
                    {editingStatus.taskStatusName || 'Preview'}
                  </span>
                </div>
