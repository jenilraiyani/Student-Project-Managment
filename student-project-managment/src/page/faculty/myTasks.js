...
const FacultyTasks = () => {
  ...
  useEffect(() => {
    if (currentUser?.userId) fetchTasks();
  }, [currentUser?.userId]);
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      const myAllocMap = {};
      allocs.filter(a => Number(a.facultyID) === myId).forEach(a => {
        myAllocMap[a.projectAllocationID] = a;
      });

      const mapped = allTasks
        .filter(t => myAllocMap[t.projectAllocationID])
        .map(t => {
          const alloc = myAllocMap[t.projectAllocationID];
          return {
            id: t.taskID,
            title: t.taskTitle,
            project: alloc?.projectTitle || '-',
            student: alloc?.studentName || '-',
  ...
  if (loading) return <div className="p-4 text-center">Loading faculty tasks...</div>;
    ...
    <>
      <div className="container-fluid p-0">
        <div className="d-flex justify-content-between align-items-center mb-4">
          <div>
            <h4 className="fw-bold mb-1 text-dark" style={{ letterSpacing: '-0.5px' }}>My Supervised Tasks</h4>
            <nav aria-label="breadcrumb">
              <ol className="breadcrumb mb-0" style={{ fontSize: '0.85rem' }}>
                <li className="breadcrumb-item"><span className="text-muted">Home</span></li>
                <li className="breadcrumb-item active fw-semibold">Faculty Tasks</li>
              </ol>
            </nav>
          </div>
          <div className="d-flex gap-2">
            <button className="btn spms-btn-primary" onClick={() => navigate('/add-task')}>Add Task</button>
            <button className="btn btn-outline-secondary" style={{ borderRadius: '8px' }} onClick={() => navigate('/score-remark')}>
              Evaluate Scores
            </button>
          </div>
        </div>
        ...
        <div className="d-flex mb-4">
          <input
            ...
            placeholder="Search tasks, student, project..."
            value={searchTerm}
              ...
              <table className="table table-hover align-middle mb-0">
                <thead>
                  <tr>
                    ...
                    <th className="text-muted fw-bold text-uppercase small">Student</th>
                ...
                <tbody>
                  {filtered.length > 0 ? filtered.map(t => (
                    <tr key={t.id} style={{ cursor: 'pointer' }} onClick={() => navigate(`/task-detail?id=${t.id}`)}>
                      ...
                      <td className="text-secondary small">{t.student}</td>
                    ...
                    <tr>
                      <td colSpan="6" className="text-center py-5 text-muted">No supervised tasks found.</td>
                    </tr>
                  )}
                </tbody>
              </table>
      ...
      <style>{`
        ...
        .spms-btn-primary {
          background: #6B5CA5; color: #fff; font-weight: 600;
          border: none; border-radius: 8px; padding: 10px 18px;
        }
        .spms-btn-primary:hover { background: #55488c; color: #fff; }
        .task-badge {
